...
function TargetAchievement({ kpi, loading }: { kpi?: DashboardResponse["kpi"]; loading: boolean }) {
  const target = kpi?.target_value ?? 0;
  const achievement = kpi?.achievement_value ?? 0;
  const percentage = Math.min(Math.max(kpi?.achievement_pct ?? 0, 0), 100);
  const remaining = kpi?.target_remaining ?? 0;
  const money = (value: number) => formatCompactIDR(value);
  return (
    <div data-testid="sales-target-achievement" className="w-full !border-0 !bg-transparent !p-0 !shadow-none !rounded-none">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0 flex-1">
          <div className="hidden"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 text-blue-600"><Target className="h-4 w-4" /></div><div><h2 className="text-sm font-bold text-slate-900">Sales Target &amp; Achievement</h2><p className="text-xs text-slate-500">Pencapaian berdasarkan nilai Purchase Order customer tahun {kpi?.target_year ?? new Date().getFullYear()}.</p></div></div>
          <div className="h-2 overflow-hidden rounded-full bg-white/55"><div data-testid="target-progress" className="h-full rounded-full bg-blue-600 transition-all duration-700" style={{ width: loading ? "0%" : `${percentage}%` }} /></div>
          <div className="mt-1.5 flex items-center justify-between gap-3 text-[10px]"><span className="font-semibold text-blue-600">{loading ? "-" : `${kpi?.achievement_pct ?? 0}% tercapai`}</span><span className="text-slate-500">{loading ? "-" : remaining > 0 ? `${money(remaining)} tersisa` : "Target tercapai"}</span></div>
        </div>
        <div className="grid w-full grid-cols-3 gap-2 lg:w-[470px]">
          <div className="min-w-0 rounded-xl border border-blue-100/80 bg-white/65 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="truncate text-[9px] font-semibold uppercase tracking-wider text-slate-500">Target</p><p className="mt-1 truncate text-sm font-bold leading-none text-slate-900">{loading ? "-" : money(target)}</p></div>
          <div className="min-w-0 rounded-xl border border-emerald-100 bg-emerald-50/70 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="truncate text-[9px] font-semibold uppercase tracking-wider text-emerald-600">Pencapaian PO</p><p className="mt-1 truncate text-sm font-bold leading-none text-slate-900">{loading ? "-" : money(achievement)}</p></div>
          <div className="min-w-0 rounded-xl border border-amber-100 bg-amber-50/75 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="truncate text-[9px] font-semibold uppercase tracking-wider text-amber-600">Remaining</p><p className="mt-1 truncate text-sm font-bold leading-none text-slate-900">{loading ? "-" : money(remaining)}</p></div>
        </div>
      </div>
    </div>
  );
...
export default function Dashboard() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery<DashboardResponse>({ queryKey: ["dashboard", qs], queryFn: () => apiGet<DashboardResponse>(`/dashboard${qs ? `?${qs}` : ""}`), staleTime: 15_000, refetchInterval: 30_000, refetchOnWindowFocus: true, refetchOnMount: "always" });
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="relative overflow-hidden rounded-2xl border border-blue-100/80 bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-100 px-6 py-5 text-slate-900 shadow-md md:px-8 md:py-6"><div className="absolute right-0 top-0 h-full w-1/3 bg-gradient-to-l from-blue-200/70 via-sky-100/20 to-transparent" /><div className="absolute -right-16 -top-20 h-52 w-52 rounded-full bg-blue-200/30 blur-3xl" /><div className="relative">
      <div className="max-w-2xl"><div className="mb-2 inline-flex items-center gap-2 rounded-full border border-blue-200/80 bg-white/70 px-3 py-1 text-[10px] font-semibold text-blue-700 shadow-sm"><TrendingUp className="h-3 w-3" /> Business performance</div><h2 className="text-xl font-bold tracking-tight text-slate-900 md:text-2xl">Business performance at a glance</h2><p className="mt-1.5 max-w-2xl text-xs leading-5 text-slate-600">Gunakan pipeline dan indikator operasional untuk memprioritaskan follow-up, quotation, dan delivery.</p></div>
      <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(470px,0.95fr)] lg:items-end">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-xl border border-blue-100/80 bg-white/65 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="text-[9px] uppercase tracking-wider text-slate-500">Customers</p><p className="mt-1 text-base font-bold text-slate-900">{num(kpi?.total_customers)}</p></div><div className="rounded-xl border border-blue-100/80 bg-white/65 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="text-[9px] uppercase tracking-wider text-slate-500">Pipeline</p><p className="mt-1 truncate text-base font-bold text-slate-900">{cur(kpi?.open_pipeline)}</p></div><div className="rounded-xl border border-blue-100/80 bg-white/65 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="text-[9px] uppercase tracking-wider text-slate-500">Won</p><p className="mt-1 truncate text-base font-bold text-slate-900">{cur(kpi?.won_value)}</p></div><div className="rounded-xl border border-blue-100/80 bg-white/65 px-3 py-2.5 shadow-sm backdrop-blur-sm"><p className="text-[9px] uppercase tracking-wider text-slate-500">Overdue</p><p className="mt-1 text-base font-bold text-amber-600">{num(kpi?.overdue_orders)}</p></div></div>
        <TargetAchievement kpi={kpi ?? undefined} loading={isLoading} />
      </div>
    </div></section>
