...
export default function Dashboard() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="relative overflow-hidden rounded-2xl bg-slate-950 px-6 py-6 text-white shadow-lg md:px-8 md:py-7"><div className="absolute right-0 top-0 h-full w-1/3 bg-gradient-to-l from-blue-900/40 to-transparent" /><div className="relative grid gap-7 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold text-blue-200"><TrendingUp className="h-3.5 w-3.5" /> Business performance</div><h2 className="max-w-2xl text-xl font-bold tracking-tight md:text-2xl">Business performance at a glance</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Gunakan pipeline dan indikator operasional untuk memprioritaskan follow-up, quotation, dan delivery.</p></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:w-[440px]"><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">Customers</p><p className="mt-1 text-lg font-bold">{num(kpi?.total_customers)}</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">Pipeline</p><p className="mt-1 text-lg font-bold">{cur(kpi?.open_pipeline)}</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">Won</p><p className="mt-1 text-lg font-bold">{cur(kpi?.won_value)}</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">Overdue</p><p className="mt-1 text-lg font-bold text-amber-300">{num(kpi?.overdue_orders)}</p></div></div></div></section>
