...
function TargetAchievement({ kpi, loading }: { kpi?: DashboardResponse["kpi"]; loading: boolean }) {
  ...
  return (
    <Card data-testid="sales-target-achievement" className="overflow-hidden rounded-2xl border-slate-200/80 bg-white p-5 shadow-sm md:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 text-blue-600"><Target className="h-4 w-4" /></div><div><h2 className="text-sm font-bold text-slate-900">Sales Target &amp; Achievement</h2><p className="text-xs text-slate-500">Pencapaian berdasarkan nilai Purchase Order customer tahun {kpi?.target_year ?? new Date().getFullYear()}.</p></div></div>
          <div className="mt-5 h-3 overflow-hidden rounded-full bg-slate-100"><div data-testid="target-progress" className="h-full rounded-full bg-blue-600 transition-all duration-700" style={{ width: loading ? "0%" : `${percentage}%` }} /></div>
          <div className="mt-2 flex items-center justify-between gap-3 text-[11px]"><span className="font-semibold text-blue-600">{loading ? "-" : `${kpi?.achievement_pct ?? 0}% tercapai`}</span><span className="text-slate-400">{loading ? "-" : remaining > 0 ? `${money(remaining)} tersisa` : "Target tercapai"}</span></div>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:w-[560px]">
          <div className="rounded-xl border border-slate-100 bg-slate-50/70 px-3 py-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Target</p><p className="mt-1 truncate text-sm font-bold text-slate-900">{loading ? "-" : money(target)}</p></div>
          <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 px-3 py-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-emerald-600">Pencapaian PO</p><p className="mt-1 truncate text-sm font-bold text-slate-900">{loading ? "-" : money(achievement)}</p></div>
          <div className="rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-blue-600">Achievement</p><p className="mt-1 text-sm font-bold text-slate-900">{loading ? "-" : `${kpi?.achievement_pct ?? 0}%`}</p></div>
          <div className="rounded-xl border border-amber-100 bg-amber-50/60 px-3 py-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-amber-600">Remaining</p><p className="mt-1 truncate text-sm font-bold text-slate-900">{loading ? "-" : money(remaining)}</p></div>
        </div>
      </div>
    </Card>
  );
